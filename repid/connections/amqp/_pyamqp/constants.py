...
TOKEN_TYPE_SASTOKEN = "servicebus.windows.net:sastoken"  # noqa: S105
CBS_PUT_TOKEN = "put-token"  # noqa: S105
CBS_NAME = "name"
...
class SessionTransferState(int, Enum):
    OKAY = 0
    ERROR = 1
    BUSY = 2


class LinkDeliverySettleReason(int, Enum):
...
class ManagementLinkState(int, Enum):
    IDLE = 0
    OPENING = 1
    CLOSING = 2
    OPEN = 3
...
class ManagementOpenResult(int, Enum):
    OPENING = 0
    OK = 1
    ERROR = 2
    CANCELLED = 3
...
class ManagementExecuteOperationResult(int, Enum):
    OK = 0
    ERROR = 1
    FAILED_BAD_STATUS = 2
    LINK_CLOSED = 3
...
class SenderSettleMode(int, Enum):
...
class ReceiverSettleMode(int, Enum):
...
class MessageDeliveryState(int, Enum):
    WaitingToBeSent = 0
    WaitingForSendAck = 1
    Ok = 2
    Error = 3
