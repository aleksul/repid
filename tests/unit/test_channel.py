...
@pytest.mark.parametrize("limits", [object(), ActorLimits()])
def test_channel_rejects_non_message_limits(limits: object) -> None:
    with pytest.raises(TypeError, match="MessageLimits"):
        Channel(address="test_channel", limits=limits)  # type: ignore[arg-type]
