...
async def test_run_worker_applies_actor_on_oversized_payload(
    action: Any,
    ...
    @router.actor(limits=ActorLimits(max_payload_bytes=1, on_oversized_payload=action))
...
async def test_run_worker_nacks_at_fetch_when_shrunken_cap_meets_strict_action() -> None:
    ...
    @router.actor(
        channel=Channel(
            address="jobs",
            limits=MessageLimits(max_payload_bytes=100, on_oversized_payload="nack"),
        ),
