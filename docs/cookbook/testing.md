...
Repid provides a built-in `TestClient` to make testing your actors and application logic simple and
fast. You don't need any complex pytest plugins or real message brokers.

## Preparation

In the following examples, we will assume you have created an application with the following
structure:
...
The `TestClient` allows you to send messages directly to your application without needing a running
server. It intercepts the messages and can either automatically process them, or let you process
...
If you want to control exactly when messages are processed (e.g. to test queue buildup or chaining),
you can set `auto_process=False` on the test client.
...
The `TestMessage` object returned by the `TestClient` tracks the full lifecycle of the message. You
can assert against various properties to make sure your actors behave correctly:
...
Both functions optionally accept a `channel` or `operation_id` argument to filter the list of
messages.

## Unit Testing Actors

Remember that Repid doesn't modify your actor functions in any way. You can always write simple,
isolated unit tests by importing your function and calling it directly:
