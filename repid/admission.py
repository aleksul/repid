...
async def reserve_limit_policies(
    ...
    try:
        for policy, reserve in sorted(unique.values(), key=lambda request: id(request[0])):
            lease = await reserve(partial(on_wait, policy))
            leases.append(_ReservationLease(lease.release))
...
class IntakeGate:
    ...
    def native_limit_is_independent(
        ...
        """Whether this channel's native resource cap is not shared with another link."""
        limit = (
            self.native_message_limit(channel)
            if resource == "messages"
            else self.native_payload_limit(channel)
        )
        if limit is None:
            return False
        attribute = "max_messages" if resource == "messages" else "max_payload_bytes"
        subscribed_channels = tuple(channels)
        return all(
            all(
                other == channel
                or not any(policy is other_policy for other_policy in self._policies(other))
                for other in subscribed_channels
            )
            for policy in self._policies(channel)
    ...
    def native_limit_is_uniform(
        ...
        """Whether every subscribed channel resolves to the subscriber-wide native cap."""
        limit = (
            self.native_message_limit() if resource == "messages" else self.native_payload_limit()
        )
        if limit is None:
            return False
        getter = self.native_message_limit if resource == "messages" else self.native_payload_limit
        return all(getter(channel) == limit for channel in channels)

    def _policies(self, channel: str) -> tuple[MessageLimits, ...]:
        worker = (self._limits,) if self._limits is not None else ()
        return (*worker, *self._channel_limits.get(channel, ()))
    ...
    async def reserve(self, message: ReceivedMessageT) -> ReservationLeaseT:
        ...
        for policy in reversed(policies):
            is_oversized = (
                ...
                and len(message.payload) > policy.max_payload_bytes
            )
            if is_oversized:
                action = resolve_on_oversized_payload(policy.on_oversized_payload, message)
                if action != "run_alone":
...
class _BackpressureController:
    ...
    def __init__(
        ...
        *,
        ...
        self.dispatcher: SubscriberDispatcher | None = None
    ...
    def _supports_native(
        ...
        if resource is None or self.dispatcher is None:
            return False
        capabilities = self._server.capabilities
        if resource == "messages":
            knob, per_channel_knob = (
                "supports_native_message_flow_control",
                "supports_native_message_flow_control_per_channel",
            )
        else:
            knob, per_channel_knob = (
                "supports_native_payload_flow_control",
                "supports_native_payload_flow_control_per_channel",
            )
        if not capabilities.get(knob, False) and not capabilities.get(per_channel_knob, False):
            return False
        # Per-link windows cannot represent a budget shared by channels, and a
        # subscription-wide window can only represent a cap every subscribed
        # channel resolves to; either granularity is sufficient on its own.
        per_channel_available = bool(capabilities.get(per_channel_knob, False))
        global_available = bool(capabilities.get(knob, False))
        if per_channel_available and self.dispatcher.native_limit_is_independent(
            channel,
            self.channels,
            resource,
        ):
            return True
        return global_available and self.dispatcher.native_limit_is_uniform(
            self.channels,
            resource,
        )

    def _channel_pause_ready(self) -> bool:
        """Whether per-channel pauses are claimed and the subscriber implements them."""
        if self.subscriber is None:
            return False
        if not self._server.capabilities.get("supports_pause_per_channel", False):
            return False
        return callable(getattr(self.subscriber, "pause_channel", None)) and callable(
            getattr(self.subscriber, "resume_channel", None),
        )
    ...
    def _strategy_available(
        ...
        if strategy == "channel_pause":
            return self._channel_pause_ready()
        if strategy == "worker_pause":
            return self.subscriber is not None and capabilities.get("supports_pause", False)
    ...
    def can_control_without_native(self, channel: str) -> bool:
        ...
        return (
            (
                "channel_pause" in strategies
                and capabilities.get("supports_pause_per_channel", False)
            )
            or ("worker_pause" in strategies and capabilities.get("supports_pause", False))
        ...
        try:
            async with self._lock:
                ...
                if subscriber is None:
                    return tracked
                can_pause_channels = self._channel_pause_ready()
                for channel in pause_channels:
                    if can_pause_channels:
    ...
    async def shutdown(self) -> None:
        """Prevent completed reservations from reopening intake during shutdown."""
        async with self._lock:
    ...
    async def on_ready(self, channels: Iterable[str]) -> None:
        ...
        async with self._lock:
            for channel in channels:
                ...
                if subscriber is None or self._shutting_down:
                    continue
                if channel in self._paused_channels:
                    self._paused_channels.remove(channel)
                    resume_channels.append(channel)
                if self._global_paused and not self._waiters:
                    self._global_paused = False
                    global_resume = True
            if subscriber is not None:
                can_resume_channels = self._channel_pause_ready()
                for channel in resume_channels:
                    if can_resume_channels:
...
class ExecutionAdmission:
    ...
    @property
    def dispatcher(self) -> SubscriberDispatcher | None:
        return self._backpressure.dispatcher

    @dispatcher.setter
    def dispatcher(self, dispatcher: SubscriberDispatcher | None) -> None:
        self._backpressure.dispatcher = dispatcher

    def prepare(
        self,
        channels_to_actors: dict[str, list[ActorData]],
    ) -> dict[str, tuple[MessageLimits, ...]]:
        """Adopt the run's channels and return the per-channel intake limits."""
        self._channels_to_actors = channels_to_actors
        self._backpressure.channels = tuple(channels_to_actors)
        self._map_limit_policy_channels()
        self._warn_unbounded_inflight()
        prepared: dict[str, tuple[MessageLimits, ...]] = {}
        for channel, actors in channels_to_actors.items():
            ...
            policies: list[MessageLimits] = []
            if propagated_messages is not None or propagated_bytes is not None:
                policies.append(
                    MessageLimits(
                        max_messages=propagated_messages,
                        max_payload_bytes=propagated_bytes,
                        on_oversized_payload=self._intake_action(explicit),
                    ),
                )
            if explicit is not None:
                policies.append(explicit)
            if policies:
                prepared[channel] = tuple(policies)
        return prepared
    ...
    def reservations(
        ...
        for actor_limits in reversed(actor.limits):
            if (
                ...
                continue
            action = resolve_on_oversized_payload(
                actor_limits.on_oversized_payload,
                message,
            )
            if action != "run_alone":
                raise OversizedReservationError(action)
    ...
    async def shutdown_backpressure(self) -> None:
        await self._backpressure.shutdown()
    ...
    def _intake_action(
        ...
        if explicit is not None and explicit.max_payload_bytes is not None:
            return explicit.on_oversized_payload
        if self._limits.max_payload_bytes is not None:
            return self._limits.on_oversized_payload
        return "run_alone"
