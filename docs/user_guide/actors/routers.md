...
Thus, any changes made afterward have no effect -
if necessary, create a new one to materialize changed routers and actors.

## Concurrency limits

`Router(limits=)` sets built-in numeric execution limits for its actors.
`Router(limit_policies=)` adds application-defined `LimitPolicyT` instances.
Both compose with enclosing routers and `@router.actor(limits=..., limit_policies=...)`.
See [Concurrency limits](../workers/concurrency.md).
