...
def _message(payload: bytes = b"x", channel: str = "orders") -> Any:
    return SimpleNamespace(channel=channel, payload=payload, cost=1, keep_alive_interval=None)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param({"on_oversized_payload": "drop"}, id="unknown_on_oversized_payload"),
    ],
)
...
async def test_reserve_intake_applies_fixed_and_dynamic_actions() -> None:
    ...
    for policy in policies:
        ...
        dispatcher = SubscriberDispatcher(
            MessageLimits(max_payload_bytes=1, on_oversized_payload=policy),
        )
...
async def test_intake_run_alone_and_async_policy_validation() -> None:
    ...
    gate = IntakeGate(
        MessageLimits(max_payload_bytes=1, on_oversized_payload=cast(Any, async_policy)),
    )
...
async def test_intake_gate_prefers_channel_on_oversized_payload() -> None:
    intake_gate = IntakeGate(
        MessageLimits(max_payload_bytes=2, on_oversized_payload="nack"),
        {"orders": (MessageLimits(max_payload_bytes=1, on_oversized_payload="reject"),)},
    )
