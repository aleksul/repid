...
OnOversizedPayloadT: TypeAlias = OversizedPayloadAction | OversizedPayloadPolicyT
...
def validate_on_oversized_payload(value: OnOversizedPayloadT) -> OnOversizedPayloadT:
    if callable(value):
        return value
    return validate_oversized_payload_action(value)


def resolve_on_oversized_payload(
    on_oversized_payload: OnOversizedPayloadT,
    message: ReceivedMessageT,
) -> OversizedPayloadAction:
    """Resolve a fixed or message-dependent ``on_oversized_payload`` value."""
    action = (
        on_oversized_payload(message) if callable(on_oversized_payload) else on_oversized_payload
    )
    if inspect.isawaitable(action):
        if inspect.iscoroutine(action):
            action.close()
        raise TypeError("on_oversized_payload callables must be synchronous.")
...
def _validate_numeric_limits(
    ...
    validate_on_oversized_payload(on_oversized_payload)
...
class Limits:
    ...
    ``on_oversized_payload`` applies only to ``max_payload_bytes``. It may
