...
class Header(Performative):
    ...
    the ﬁelds within the header unless other target or node speciﬁc defaults have otherwise been set.
    ...
    :param bool durable: Specify durability requirements.
        ...
        A target which is not capable of fulﬁlling this guarantee MUST NOT accept messages where the durable
    ...
    :param int priority: Relative Message priority.
        This ﬁeld contains the relative Message priority. Higher numbers indicate higher priority Messages.
    ...
    CODE: ClassVar[int | bytes] = 0x00000070
...
class Properties(Performative):
    ...
    :param message_id: Application Message identiﬁer.
        Message-id is an optional property which uniquely identiﬁes a Message within the Message system.
    ...
    :param to: The address of the Node the Message is destined for.
        The to ﬁeld identiﬁes the Node that is the intended destination of the Message. On any given transfer
        this may not be the Node at the receiving end of the Link.
    :param str subject: The subject of the message.
        A common ﬁeld for summary information about the Message content and purpose.
    :param reply_to: The Node to send replies to.
        The address of the Node to send replies to.
    :param correlation_id: Application correlation identiﬁer.
        This is a client-speciﬁc id that may be used to mark or identify Messages between clients.
    :param bytes content_type: MIME content type.
        ...
        a charset parameter deﬁning the character encoding used: e.g. 'text/plain; charset="utf-8"'.
    ...
    :param bytes content_encoding: MIME content type.
        The Content-Encoding property is used as a modiﬁer to the content-type. When present, its value indicates
        what additional content encodings have been applied to the application-data, and thus what decoding
        mechanisms must be applied in order to obtain the media-type referenced by the content-type header ﬁeld.
    ...
    :param str group_id: The group this message belongs to.
        Identiﬁes the group the message belongs to.
    ...
    CODE: ClassVar[int | bytes] = 0x00000073
...
class AnnotatedMessage(BareMessage):
    ...
    :param ~uamqp.message.Header header: Transport headers for a Message.
        ...
        network. If the header section is omitted the receiver MUST assume the appropriate default values for
        the ﬁelds within the header unless other target or node speciﬁc defaults have otherwise been set.
    :param dict delivery_annotations: The delivery-annotations section is used for delivery-speciﬁc non-standard
        ...
        effects (such as any implied propagation) cannot be acted upon. Annotations may be speciﬁc to one
        implementation, or common to multiple implementations. The capabilities negotiated on link attach and on
        the source and target should be used to establish which annotations a peer supports. A registry of deﬁned
    ...
    :param dict message_annotations: The message-annotations section is used for properties of the message which
        ...
        are explicitly augmented or modiﬁed (e.g. by the use of the modified outcome).
        ...
        ﬁltered on. A registry of deﬁned annotations and their meanings can be found here:
    ...
    delivery_annotations: dict | None = None
    message_annotations: dict | None = None
