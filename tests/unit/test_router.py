...
@pytest.mark.parametrize("limits", [object(), MessageLimits()])
def test_router_rejects_non_actor_limits(limits: object) -> None:
    with pytest.raises(TypeError, match="ActorLimits"):
        Router(limits=limits)  # type: ignore[arg-type]


@pytest.mark.parametrize("limits", [object(), MessageLimits()])
def test_actor_rejects_non_actor_limits(limits: object) -> None:
    router = Router()
    with pytest.raises(TypeError, match="ActorLimits"):

        @router.actor(limits=limits)  # type: ignore[call-overload]
        async def invalid_actor() -> None:
            pass
