...
A serializer in Repid is simply a function that takes an arbitrary Python object and converts it into `bytes`. These bytes are then sent as the message payload to your broker.

## The `SerializerT` Protocol

The `SerializerT` protocol is very straightforward. It's a callable that takes `Any` data and returns `bytes`.
...
You can configure your custom serializer globally when instantiating the Repid instance or on a per-message basis.
